...
class ProcessSalaries:
    def __init__(self, file_name: str):
        ...
        self.__available_currencies = list(pd.read_sql("SELECT * from currencies WHERE date='2003-01'",self.__con).keys()[1:])
    ...
    def get_salary(self, row: pd.DataFrame) -> float or str:
        ...
        if salary_currency != 'RUR' and salary_currency in self.__available_currencies:
            date = published_at[:7]
            multi = pd.read_sql(f"SELECT {salary_currency} from currencies WHERE date='{date}'",self.__con)[f'{salary_currency}'][0]
    ...
    def csv_to_bd(self, file_name: str) -> None:
        df = pd.read_csv(file_name)
        conn = sqlite3.connect('all_vacancies')
        curs = conn.cursor()
        curs.execute('CREATE TABLE IF NOT EXISTS all_vacancies (name text, area_name text, published_at text, salary number)')
        conn.commit()
